...
#'   biological specimen.
#'
#' @param name the character string containing a name.
#' @param noName standard notation for missing names in \code{name}.
#'
#' @return A character string in lower case, with the last name provided in \code{name}.
#'
...
#'
#'   If only one name is given, the function returns the same name in lower
#'   case.
#'
#' @author
...
lastName <- function(name,
  ...
  # missing names
  if (name == "NA" | is.na(name) | name == noName)
    return(name) # stop in the case of NA as entry names
