...
#'
#' @return x with the edited geographical coordinates.
#'
#' @details For the time being the function does not differentiate missing coordinates
#' from those provided in a format that `prepCoord` does not handle. It also does not
#' differentiate original coordinates in decimal degrees in a good format from those
#' that needed to be re-formatted.
#'
#' @author Lima, R.A.F.
#'
#' @examples
...
prepCoord <- function(x) {
  ...
  ## check input
    ...
    if (sum(grepl("Latitude|Longitude", colnames(x))) < 2) { stop("input object needs to have at least: decimalLatitude or verbatimLatitude, decimalLongitude or verbatimLongitude") }
  ...
  ## Any coordinate on non-decimal degrees format?
    tmp = sum(table(lat)) - sum(table(as.double(lat)))
    if(tmp > 0) {
      lat.ref = grepl("s|S", lat) & !grepl("n|N", lat)
      long.ref = grepl("w|W|o|O", long) & !grepl("e|E|l|L", long)
      lat = gsub('\'|\"|°|º|\\*|\\|', " ", lat)
      long = gsub('\'|\"|°|º|\\*|\\|', " ", long)
      lat = gsub('[a-z]', " ", lat, ignore.case=T)
      long = gsub('[a-z]', " ", long, ignore.case=T)
      lat = gsub('   ', ' ', lat)
      long = gsub('   ', ' ', long)
      lat = gsub('  ', ' ', lat)
      long = gsub('  ', ' ', long)
