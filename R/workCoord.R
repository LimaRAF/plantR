...
#' @description
#'
#' @param x a data.frame containing the original coordinates and the coordinates obtained from a
#' gazetteer.
#'
#' @return x with the columns 'decimalLatitude.new' and 'decimalLongitude.new' filled with the
#' geographical coordinates of the gazetteer at the best resolution (i.e. administrative level) available.
#'
#' @details
#'
#' @author Lima, R.A.F.
...
workCoord <- function(x) {
  ...
  ## Creating the working coodinates and defining the origin of the information
    ...
    # Assessing the resolution of the original coordinates
      ...
      lat3[!lat3 %in% "degrees_only" &
             sapply(tmp1, function(x) x[2]) %in% c(mins1, 0)] = "minutes_only"
      long3[!long3 %in% "degrees_only" &
              sapply(tmp2, function(x) x[2]) %in% c(mins1,0)] = "minutes_only"
      # seconds
      ...
      lat3[!lat3 %in% c("degrees_only", "minutes_only") &
             sapply(tmp1, function(x) x[2]) %in% secs] = "seconds"
      long3[!lat3 %in% c("degrees_only","minutes_only") &
              sapply(tmp2, function(x) x[2]) %in% secs] = "seconds"
      # seconds with rounding issues?
      lat3[!lat3 %in% c("degrees_only", "minutes_only", "seconds") &
             nchar(sapply(tmp1, function(x) x[2])) > 6] = "seconds?"
      long3[!long3 %in% c("degrees_only", "minutes_only", "seconds") &
             nchar(sapply(tmp2, function(x) x[2])) > 6] = "seconds?"
  ...
  ##Replacing missing coordinates by the county coordinates from the gazetter
    x1$decimalLatitude.new[is.na(lat)] = x1$latitude.gazetteer[is.na(lat)]
    x1$decimalLongitude.new[is.na(long)] = x1$longitude.gazetteer[is.na(long)]
    x1$origin.coord[!is.na(x1$decimalLatitude.new) & is.na(x1$origin.coord)] = "coord_gazet"
    ...
    #x1$origin.coord[x1$origin.coord %in% "coord_gazet" & x1$resolution.gazetteer %in% "country"] = "ok_country_gazet"
    #x1$origin.coord[x1$origin.coord %in% "coord_gazet" & x1$resolution.gazetteer %in% "state"] = "ok_state_gazet"
    #x1$origin.coord[x1$origin.coord %in% "coord_gazet" & x1$resolution.gazetteer %in% "county"] = "ok_county_gazet"
    #x1$origin.coord[x1$origin.coord %in% "coord_gazet" & x1$resolution.gazetteer %in% "locality"] = "ok_locality_gazet"

  ## Preparing the output
    x2 = x[, !(names(x) %in% c("decimalLatitude.new","decimalLongitude.new"))]
    x3 = cbind.data.frame(x2, x1[, c("decimalLatitude.new","decimalLongitude.new","origin.coord","resolution.coord"),])
    return(x3)
}
