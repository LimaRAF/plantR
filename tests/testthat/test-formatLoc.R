...
df$locality <- NA
expected.state <- c(NA, NA, NA, NA, "sao paulo")
expected.resolution <- c(rep("country", 4), "state")
...
expected.locality <- "estacao biologica do alto da serra de paranapiacaba"
expected.loc.correct <- "brazil_sao paulo_santo andre_rebio alto serra paranapiacaba"
...
test_that("formatLoc fixes names correctly", {
  ...
  # expect_equal(run_test$resolution.gazetteer, expected.resolution) # descomentar após atualizar o gazeteiro
  # expect_equal(run_test$loc.correct, expected.loc.correct)) # descomentar após atualizar o gazeteiro
})
