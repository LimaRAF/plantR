...
(see the section [Brief code summary] below). But first, we will explain in
...
The package __plantR__ can be installed and loaded from [GitHub](https://github.com/LimaRAF/plantR) with the following codes (this may take a minute or two):
```{r, eval = FALSE}
...
For this tutorial, we will also need the global taxonomic backbones available from the companion R package __plantRdata__. Installing this package takes 10-15 minutes because it stores some large files (>10 MB). But this step is done just once.
```{r, eval = FALSE}
