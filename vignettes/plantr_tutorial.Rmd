...
Botanical collections around the world are digitalized and become available through data repositories. This unprecedented availability of raw data contributes to the expansion of biodiversity informatics studies. Raw data have some important sources of variation, and neither collections nor repositories perform detailed data cleaning, formatting and stantardizing, leaving these steps to the final users. 
Package __plantR__ was developped thinking about this lack of uniformity in herbarium data, to help final users and Herbaria curators process and standardize their herbaria data.

<!-- /Rbaria/curatoR/ --> 

## Sources of variation 
The information on each specimen is the responsability of the collector, who decides which information is relevant and records the collection locality with the precision available.
Traditions regarding information that is recorded have changed with time, and local toponyms have changed in time too. The quality of the georeferencing has evolved and nowadays it is a common practice to record coordinates taken with a GPS.

Although GBIF gathers information for collections all around the world, it does
not perform correction of the source data.

+ GBIF large totalizing project gathers data from collections around the world
 + no correction from source
 + no common format in spite of standard (darwinCore)
 + changing standard (DarwinCore) versions

+ collectors
  + each collector decides what info goes to the exsiccata
  + changing traditions with time
  + changing toponyms (names of localities)
  + better georeferencing
  + human errors

+ collections
  + collection managing and digitalizing is a hard task, often led by underfunded institutions
  + different decisions when digitalizing: which fields ($)
  + chronically underfunded: no way to update periodically what has been digitalized or to update to a changing standard
  + collection size is prohibitive
  + different collections apply different quality filters to their data
  + human errors

### Other sources of variation in biodiversity collection data

+ Evolving taxonomy: synonyms
+ Typographic errors
+ Multiplicity of notations regarding author names, localities, taxa names.
+ Lack of gazetteers
...
__plantR__ is an R package meant to format, standardize and validate the information that comes from different biodiversity databases, such as GBIF, species link, and herbaria collections in general. __plantR__ will return standardized information from the original database, correcting for differences in each field and allowing for comparison between databases.

The package can be used by collection curators that wish to standardize the fields after digitalization and to the databases final users, that wish to compare information between collections, for example when information is downloaded from GBIF.


# Workflow overview

The workflow is structured in four steps that are:

+ data download,
+ data formatting: fields, names and locality formatting
+ geographic validation
+ taxonomical validation.

## Source data

The source data that compose this package <!-- a fonte criação e formatação de todas as bases de dados deveria estar documentada
 descrita e se possível justificada-->

 ## Dados: (reordenar à vontade)
 + admin
 + collectionCodes
 + field_name
 + replace_names
...
download, editing and validation using __plantR__.
For each of these steps, there is a more specific tutorial presenting all functionalities more in detail (see package vignettes).  <!-- eu acho que o detalhe interno é o que deve ir nesses tutoriais --> 
...
Particularly when working with data coming from multiple secondary sources, it is important
...
## Editing geographical coordinates: functions `prepCoord()` and `workCoord()`
...
## Editing species names: function `prepSpecies()`
...
For our example we decided to replace all edited species names (i.e. 'scientificName.new') by the suggested species names from the function `formatSpecies` (i.e. 'scientificName.new'). However,
__plantR__ does not execute this step automatically and the decision to do it should be taken by the user:
...
This is not the optimal example because Gerdt (Guenther) Hatschbach was not a Salicaceae specialist. However, he provided determinantions for many different families outside his speciality, often referred to as a generalist. There are some names of generalists in the __plantR__ default taxonist database, and those names can be included in the taxonomic validation through the arguments `generalist`. However, this list of generalists is biased towards South America, particularly Brazil.
...
Note that we used the defaults of the `prepDup` function, which exclude from the search the search strings without collector name, number or year (e.g. 's.n.' or 's.d.'), as well as missing information for these columns (i.e. NAs). If one decided to include those strings in the search, the number of specimens that will pass the duplicate search will increase but the chances of finding false duplicates (e.g. Salicaceae + s.n. + s.n. + Ubatuba or Salicaceae + s.n. + s.n. + s.d.) increase as well.
...
table(dups$dup.prop)
```
As a result we found about 5300 specimens with a high chance of representing true duplicates, i.e. the porpotion of search strings shred between them are maximum (dup.prop = 1). Other ~300 specimens had only one search string in common. Moreover, we have indication of possible ~4600 unicates, and for other ~6800 specimens we could not check for dupicates (i.e. all search strings used have missing information).

Note as well that if there are duplicated entries (i.e. same acession number from the same collection) both are listed within the accession number grouped in the new column 'dup.ID'.

It is important to notice that these results are only indications. First of all, the retrievel of duplicates depend on the dataset itself: if the collection were the dupliate was deposited is not within the dataset (e.g. collection is not in speciesLink or GBIF), the duplicate cannot be retrieved for obvious reasons. But other issues may change the number of unicates and duplucates found. If the search string is too flexible (e.g. only collector name and number), false duplicates may be retrieved if collector names are common (e.g. 'Silva', 'Santos' or 'Lima' for Portuguese family names) and collector numbers are low. So having more than one search string with at least 3 combinations of information descreases the chnce of false duplicates. On the other hand, we may have false unicates due to differences in the notation among collections, which could not be solved by __plantR__ (e.g. Mattos-Silva, L.A. vs. Silva, L.A.M.), or typos (e.g. Hoffmannsegg, J.C. vs. Hoffmannseg, J.C.). Finally, many duplicates may still exist but due to missing information (e.g. collecor number or collection year or locality), they could not enter the duplicate search.  
...
Next, we use function `mergeDup` to perform the homogenization itself, which aims at expanding the information with highest quality found across all specimens contained in the groups of duplicates identified above. 
...
As an example, we show that the function was able to retrieve species identifications
with a high confidence level for about 700 specimens. Tehre was also an improvement on the stats regarding the geographical and locality checks (i.e. columns 'geo.check' vs. 'geo.check1' and 'loc.check' vs. 'loc.check1')
