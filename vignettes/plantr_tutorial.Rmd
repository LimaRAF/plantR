...
vignette: >
  ...
  %\VignetteIndexEntry{plantR_tutorial}
...
As expected, only ca. 15% of the specimesn were identified by a family specialist. Note that the function also returns a list with up to 10 names in the TDWG format that have more identifications. Maybe this name could represent a missing taxonomist from the __plantR__ database of plant taxonomists. If this is the case, one can add taxonomist to the validation using the argument `miss.taxonomist`:
...
`prepDup` basically creates concatenated strings with different set of species information, which will be used in the search. Different combinations of fields can be used to generate these search strings, through the argument `comb.fields`:
...
Next, we use function `mergeDup` to perform the homogenization itself, which aims at expanding the highest-quality information found across all specimens of each group of duplicate. 
