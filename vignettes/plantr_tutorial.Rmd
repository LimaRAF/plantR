...
```{r, eval = TRUE}
occs$recordedBy.new <- unlist(sapply(occs$recordedBy.new, formatName))
occs$identifiedBy.new <- unlist(sapply(occs$identifiedBy.new, formatName))
```
...
## Editing species names: function `formatTaxa()`

<!--Species names spelling - tipográficos
format/modificators
synonyms
replace_names/suggest_name-->

Suggestion of items adapted from the original workflow:

- [x] 4.0 First edit (flag indets, class taxon rank, remove authors)
- [x] 4.1 Remove cf. e aff. (and create a clomun for the name modificators?)
- [x] 4.2 Option to consider or not names at infraspecific level at specific level (e.g. remove ‘var.’, ‘subsp.’, ‘forma’, etc)
...
In __plantR__ we offer a new function that perform all these step and an extra step, which is the extraction
...
Note that there are more species names than the ones used to search for occurences. Some are due to name format and other are due to differences in the assignnemt of synonyms between speciesLink and the Brazilian Flora interface used here, the package __flora__.
